...
def post_process_data(conn):
    ...
    category_to_group_mapping = {subcat: maincat for maincat, sublist in category_mapping.items() for subcat in sublist}
    conn.execute('ALTER TABLE crypto_merged ADD COLUMN "group" VARCHAR;')
    for subcat, maincat in category_to_group_mapping.items():
        conn.execute(f'UPDATE crypto_merged SET "group" = \'{maincat}\' WHERE category = \'{subcat}\';')
    conn.execute("PRAGMA memory_limit='32GB'")
    category_to_type_mapping = {subcat: maincat for maincat, sublist in category_mapping.items() for subcat in sublist}
    conn.execute('ALTER TABLE crypto_merged ADD COLUMN "type" VARCHAR;')
    for category, type_ in category_to_type_mapping.items():
        conn.execute(f'UPDATE crypto_merged SET "type" = \'{type_}\' WHERE category = \'{category}\';')
    conn.execute("""UPDATE crypto_merged SET chainTvls = REPLACE(chainTvls, $$'$$, $$"$$);""")
