# Network of TVL Value Transfers
...
const data = FileAttachment("./data/fandoms2010-10.json").json(); 
const nw = FileAttachment("./data/network/2023-06-23.json").json();
```
...
const numberOfMonths = d3.timeMonth.count(timemin, timemax);
const timeScale = d3.scaleTime()
  .domain([timemin, timemax])
	.range([0, numberOfMonths]);

const windowspan = 2;
const date_compare = timeScale.invert(numberOfMonths);
const datewindow = timeScale.invert(numberOfMonths - windowspan);
...
function slider(){
  let slider = html`
    <label for="threshold" style="width:25%;display:inline-block">Date: ${formatTime(
      timeScale.invert(numberOfMonths)
    )}</label>
    <input type="range" min="0" max="${numberOfMonths}" style="width:100%;display:inline-block"></input>`;
  let label = slider.querySelector('label');
  let range = slider.querySelector('input');
  range.value = numberOfMonths;
  range.oninput = () => {
    let threshold = range.valueAsNumber;
    label.innerHTML = `Date: ${formatTime(timeScale.invert(threshold))}`;
    
    var c_value = formatMonth(timeScale.invert(threshold));
    var centrality_value = centrality + c_value;
    var date_compare = formatMonth(timeScale.invert(threshold));
    var datewindow = formatMonth(timeScale.invert(threshold-windowspan))
    ...
    // let newNodes = data.nodes.filter(
    //   n => n.hasOwnProperty(centrality_value) 
    // );
    
    let newLinks = data.links.filter(
      l => (parseTime(l.Date) <= parseTime(date_compare)) && (parseTime(l.Date) >= parseTime(datewindow))
    );
    // console.log("nodes", newNodes.length, "links", newLinks.length);
